...
export class ProjectDetailComponent implements OnInit {
  ...
  deleteCategoryInProjectCollection(categories, categorie, arrayCat) {
      ...
      if (doubleCategoryInProject) {
        const index2 =  this.categoriesListInCollection[0].categories.indexOf(categoryId);
        this.categoriesListInCollection[0].categories.splice(index2);
        this.ms.addCategories(categories, categoryId);
      }
  ...
  verifyIfCategoryExistInProjectCollection(categorie) {
    const index2 =   this.getAllCategoriesInProjectCollection().indexOf(categorie);
      if (index2) {
        for ( let i = index2 + 1 ; i <  this.getAllCategoriesInProjectCollection().length ; i++  ) {
            if ( this.getAllCategoriesInProjectCollection()[i] === categorie ) {
                this.doublecategory =  this.getAllCategoriesInProjectCollection()[i];
                break ;
            } else {
              this.doublecategory = null ;
            }
        }

    }
    return this.doublecategory;
  }
  remove(categories,categorie, arrayCat): void {
    this.deleteCategoryInProjectCollection(categories, categorie, arrayCat);
    const category = this.verifyIfCategoryExistInProjectCollection(categorie);
    console.log(this.verifyIfCategoryExistInProjectCollection(categorie));
  }
