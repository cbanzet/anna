...
export class ProjectDetailComponent implements OnInit {
  ...
  filterClients(val: string) {
    ...
    return this.clients
    .map(response => response.filter(client => {     
        if(client.lastname) return (client.lastname.toLowerCase().indexOf(val.toLowerCase()) === 0)   
        else return null;
    }));
  }
  ...
  displayFn(client) {
    if(client) {
      var name = `${client.lastname}`;      
    }
    else var fullname = '';
    return client ? lastname : client;
  }
  ...
  verifyIfCategoryExistInProjectCollection(categorie) {
    console.log( this.getAllCategoriesInProjectCollection());
        var tab = [] ;
        this.getAllCategoriesInProjectCollection().forEach((val) => {
           if (val === categorie ) {
             tab.push(val);
           }
        });
  ...
  deleteCategoryInProjectCollection(categories, categorie, arrayCat) {
      const reponse = this. verifyIfCategoryExistInProjectCollection(categorie);
