...
export class ProjectDetailComponent implements OnInit {
  ...
  remove(categories,categorie, arrayCat): void {
    ...
    const id = arrayCat[0].id ;
    const categoriesListInCollection = [] ;
    // if (index >= 0) {
   ...
   this.allCategories.forEach((val) => {
      val.categories.forEach((element) => {
        // const index2 = categories.indexOf(categorie);
      //  console.log(element);
        categoriesListInCollection.push(element) ;
      });
   });
   const index2 = categoriesListInCollection.indexOf(categorie);
   console.log(categoriesListInCollection);
  }
